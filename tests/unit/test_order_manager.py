...
def make_intent(**kwargs):
    defaults = {"intent": IntentType.ADD_ITEM}
...
def test_add_item_defaults(mgr):
    intent = make_intent(item_id="classic_burger")
    item = mgr.add_item(intent)
...
def test_add_item_quantity(mgr):
    intent = make_intent(item_id="fries", quantity=2)
    item = mgr.add_item(intent)
...
def test_add_item_invalid_id(mgr):
    with pytest.raises(OrderError, match="Unknown menu item"):
        mgr.add_item(make_intent(item_id="unknown_item"))


def test_add_item_invalid_extra(mgr):
    with pytest.raises(OrderError, match="Invalid extra"):
        mgr.add_item(make_intent(item_id="classic_burger", extras_add=["mushrooms"]))


def test_add_item_invalid_option(mgr):
    with pytest.raises(OrderError):
        mgr.add_item(make_intent(item_id="classic_burger", options={"size": "xl"}))


def test_add_item_sets_defaults_for_required(mgr):
    intent = make_intent(item_id="soda")
    item = mgr.add_item(intent)
...
def test_modify_item_change_size(mgr):
    mgr.add_item(make_intent(item_id="classic_burger"))
    intent = make_intent(intent=IntentType.MODIFY_ITEM, item_id="classic_burger", options={"size": "large"})
    item = mgr.modify_item(intent)
...
def test_modify_item_add_extra(mgr):
    mgr.add_item(make_intent(item_id="classic_burger"))
    intent = make_intent(intent=IntentType.MODIFY_ITEM, item_id="classic_burger", extras_add=["cheese"])
    item = mgr.modify_item(intent)
    assert "cheese" in item.extras
...
def test_modify_item_remove_extra(mgr):
    mgr.add_item(make_intent(item_id="classic_burger", extras_add=["cheese", "bacon"]))
    intent = make_intent(intent=IntentType.MODIFY_ITEM, item_id="classic_burger", extras_remove=["bacon"])
    item = mgr.modify_item(intent)
...
def test_modify_item_change_quantity(mgr):
    mgr.add_item(make_intent(item_id="fries"))
    intent = make_intent(intent=IntentType.MODIFY_ITEM, item_id="fries", quantity=3)
    item = mgr.modify_item(intent)
...
def test_modify_item_resolves_last_item(mgr):
    mgr.add_item(make_intent(item_id="fries"))
    intent = make_intent(intent=IntentType.MODIFY_ITEM, options={"size": "large"})
    item = mgr.modify_item(intent)
    assert item.options["size"] == "large"
...
def test_modify_item_ambiguous_raises(mgr):
    ...
    with pytest.raises(OrderError, match="2 Classic Burgers"):
        mgr.modify_item(intent)


def test_modify_empty_order_raises(mgr):
    intent = make_intent(intent=IntentType.MODIFY_ITEM, item_id="fries")
...
def test_remove_item(mgr):
    mgr.add_item(make_intent(item_id="classic_burger"))
    mgr.add_item(make_intent(item_id="fries"))
    intent = make_intent(intent=IntentType.REMOVE_ITEM, item_id="fries")
    removed = mgr.remove_item(intent)
...
def test_remove_only_item(mgr):
    mgr.add_item(make_intent(item_id="classic_burger"))
    intent = make_intent(intent=IntentType.REMOVE_ITEM, item_id="classic_burger")
    mgr.remove_item(intent)
...
def test_remove_by_index(mgr):
    mgr.add_item(make_intent(item_id="classic_burger"))
    mgr.add_item(make_intent(item_id="fries"))
    intent = make_intent(intent=IntentType.REMOVE_ITEM, target_index=0)
    removed = mgr.remove_item(intent)
...
def test_calculate_price_pizza_small(mgr):
    intent = make_intent(item_id="margherita", options={"size": "small"})
    item = mgr.add_item(intent)
...
def test_calculate_price_milkshake_large_with_extras(mgr):
    intent = make_intent(
        ...
        extras_add=["whipped_cream", "cherry_on_top"],
    )
    item = mgr.add_item(intent)
...
def test_to_submit_payload(mgr):
    mgr.add_item(make_intent(item_id="classic_burger", options={"size": "large"}, extras_add=["cheese"]))
